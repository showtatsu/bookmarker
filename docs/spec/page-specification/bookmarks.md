...
| 要素             | 説明                                                               |
| ---------------- | ------------------------------------------------------------------ |
| 検索ボックス     | タイトル、URL/パス、説明文を全文検索。ページ表示時に自動フォーカス。右端にクリアボタン（×）を表示し、クリックで入力テキストをクリア |
...
- タグ入力テキストボックスの下に既存タグ一覧をドロップダウン形式で表示
- インクリメンタルサーチ対応：入力文字列に部分一致するタグを候補として絞り込み表示
- 候補タグをクリックまたは Enter で選択し、タグとして追加
- 候補が無い場合は「新規タグとして追加」オプションを表示
- 選択済みタグは入力欄の左側にチップ形式で表示し、×ボタンで削除可能
